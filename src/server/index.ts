//这个文件是Cursor Tutor的服务器端入口文件，它负责启动HTTP服务器和WebSocket服务器，并注册中间件。此外，它还负责加载平台特定的模块，如Google设备和Apple设备的控制中心、设备跟踪器、远程Shell等。当接收到SIGINT或SIGTERM信号时，它会释放所有正在运行的服务并退出进程。
import '../../LICENSE';
